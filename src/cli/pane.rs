...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: herdr pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE] [--terminal-id ID] [--process-group-id PID] [--origin-pid PID]";
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--terminal-id" | "--process-group-id" | "--origin-pid" => {
                ...
                } else if option == "--process-group-id" {
                    process_group_id = Some(
                        super::parse_u64_flag(option, value)?
                            .try_into()
                            .map_err(|_| std::io::Error::other("invalid process group id"))?,
                    );
                } else {
                    origin_pid = Some(
                        super::parse_u64_flag(option, value)?
