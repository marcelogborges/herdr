...
impl App {
    ...
    pub(super) fn handle_pane_report_agent_session(
        ...
        let foreground_job = params.process_group_id.and_then(|_| {
            terminal_id
        ...
        if params
            .terminal_id
            .as_deref()
            .is_some_and(|expected| terminal_id.map(|id| id.as_str()) != Some(expected))
            || params.process_group_id.is_some_and(|expected| {
                foreground_job.as_ref().map(|job| job.process_group_id) != Some(expected)
            })
        {
            return encode_error(
                id,
                "agent_session_stale",
                "the terminal process changed before the session report",
            );
        }
        let Some(agent_label) = normalize_reported_agent_label(&params.agent) else {
            return invalid_agent(id);
        ...
        let transcript_admitted = codex_report
            && params.terminal_id.is_some()
            && params.process_group_id.is_some()
            && params.origin_pid.is_some();
        if codex_report && params.origin_pid.is_some() && !transcript_admitted {
            return encode_error(
                id,
                "agent_session_stale",
                "Codex hook origin is not verified",
            );
        }
