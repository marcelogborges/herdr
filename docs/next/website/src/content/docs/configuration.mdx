...
The panel opens in the worktree the focused Claude Code session works in when
Herdr can tell, and otherwise in the focused pane's directory. Herdr keeps one
panel terminal per tab and directory, so switching agents switches the panel
too and returning restores where you left off. Clicking the panel focuses it;
clicking a pane or pressing the prefix works as usual. Drag the panel's left
edge to resize it; double-click the edge to return to `right_panel_width`. The
dragged width lasts until the server restarts. Quitting the panel
command closes that terminal and hides the panel.
