...
`right-panel open` は右パネルをファイルモードで表示してフォーカスします。ファイルの場合は、そのファイルのディレクトリ用の新しいパネル端末で `[right_panel] open_command`（既定値 `micro +{line} {path}; exec yazi {path}`）を実行します。ディレクトリの場合はそこでファイル表示を開きます。相対パスは現在のディレクトリを基準に解決され、`--line` がない場合は `PATH:LINE` も使えます。別のペインやタブにフォーカスするかパネルを切り替えるまで、パネルは開いたファイルに留まります。ペイン内で実行すると `HERDR_PANE_ID` が自動で渡されます。
