...
`right-panel open` shows the right panel in files mode and focuses it. For a file it runs `[right_panel] open_command` (default `micro +{line} {path}; exec yazi {path}`) in a new panel terminal for the file's directory; for a directory it shows the files view there. Relative paths resolve against the current directory, and `PATH:LINE` works when `--line` is not given. The panel stays on the opened file until you focus another pane or tab, or toggle the panel. Run inside a pane, the command passes `HERDR_PANE_ID` automatically.
